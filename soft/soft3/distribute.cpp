...
void chassis::update(Subject *subject)
{
    std::string state = subject->getState();
    cout<<"The obstacle is "<<state<<" of me, then the chassis performs "<<state2action[state]<<"."<<std::endl;
}
...
std::string raidar::getState()
{
    return this->state;
}

void raidar::setState(std::string s)
{
    this->state = s;   
}
...
void smartCar::raidarSetState(std::string s)
{
    this->raidar_.setState(s);
}

void smartCar::raidarAttachChassis()
{
    this->raidar_.attach(&this->chassis_);
}

void smartCar::raidarDetachChassis()
{
    this->raidar_.detach(&this->chassis_);
}

void smartCar::raidarNotify()
{
    this->raidar_.Notify();
}
